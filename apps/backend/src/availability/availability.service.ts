...
import { ProviderAvailability, ProviderAvailabilityDocument } from './schemas/provider-availability.schema';
...
export class AvailabilityService {
  constructor(
    @InjectModel(ProviderAvailability.name) private availabilityModel: Model<ProviderAvailabilityDocument>,
    @InjectModel(TenantStaff.name) private staffModel: Model<TenantStaffDocument>,
  ) {}

  async getAvailabilitySlots(tenantId: string, start: Date, end: Date): Promise<AvailabilitySlot[]> {
    ...
    const providerIds = availability
      .map((a) => {
        const p = (a as { providerId?: { _id?: unknown } | Types.ObjectId }).providerId;
        if (p && typeof p === 'object' && '_id' in p) return (p as { _id: Types.ObjectId })._id;
        return p as Types.ObjectId;
      })
      .filter(Boolean);
    const staff = await this.staffModel
      .find({ _id: { $in: providerIds }, tenantId: new Types.ObjectId(tenantId) })
    ...
    while (cur <= endDate) {
      ...
      for (const av of availability) {
        const doc = av as { providerId?: { _id?: unknown }; dayOfWeek?: number; startTime?: string; endTime?: string };
        ...
        const id = prov && typeof prov === 'object' && '_id' in prov ? String(prov._id) : '';
