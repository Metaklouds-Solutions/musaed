...
export class ExportService {
  ...
  async exportStaffCsv(tenantId: string): Promise<string> {
    const staff = await this.staffModel
      .find({ tenantId: new Types.ObjectId(tenantId), status: { $ne: 'disabled' } })
      .populate('userId', 'email name')
      .populate('tenantId', 'name')
      .lean();

    const headers = ['Email', 'Name', 'Role', 'Status', 'Tenant'];
    const rows = (staff as { userId?: { email?: string; name?: string }; tenantId?: { name?: string }; roleSlug?: string; status?: string }[]).map(
      (s) => [
  ...
  async exportTicketsCsv(tenantId: string): Promise<string> {
    ...
    const headers = ['ID', 'Title', 'Category', 'Status', 'Priority', 'Created'];
    const rows = (tickets as { _id?: unknown; title?: string; category?: string; status?: string; priority?: string; createdAt?: Date }[]).map(
      (t) => [
  ...
  async exportTenantsCsv(): Promise<string> {
    ...
    const headers = ['ID', 'Name', 'Slug', 'Status', 'Owner Email', 'Owner Name'];
    const rows = (tenants as { _id?: unknown; name?: string; slug?: string; status?: string; ownerId?: { email?: string; name?: string } }[]).map(
      (t) => [
        escapeCsv(t._id != null ? String(t._id) : ''),
        escapeCsv(t.name),
        escapeCsv(t.slug),
