...
## What Is MUSAED / Clinic CRM?

**MUSAED** is a **multi-tenant SaaS platform** for clinics that combines **AI voice agents** (Retell integration), **staff management**, **call handling**, **bookings**, **billing**, and **support**. It has two distinct portals:

| Portal | Audience | Purpose |
|--------|----------|---------|
| **Admin Portal** | Platform operators (ADMIN role) | Manage tenants, agents, cross-tenant calls, billing, system health, support tickets |
| **Tenant Portal** | Clinic staff (TENANT_OWNER, STAFF) | Dashboard, calls, agent, staff, reports, help center, bookings, customers, billing, settings |
...
│  /admin/* — Overview, Tenants, Agents, Staff, Support, Calls,   │
│  Runs, Skills, Billing, Settings, System                        │
└─────────────────────────────────────────────────────────────────┘
                              │
┌─────────────────────────────────────────────────────────────────┐
│  Tenant Boundary (tenantId)                                     │
│  Tenant Portal — Dashboard, Calls, Agent, Staff, Reports,       │
│  Help Center, Bookings, Customers, Alerts, Billing, Settings    │
└─────────────────────────────────────────────────────────────────┘
                              │
┌─────────────────────────────────────────────────────────────────┐
│  Data Layer                                                     │
│  Adapters → seedData (local) / API (future)                     │
│  adminAdapter, staffAdapter, agentsAdapter, supportAdapter,     │
│  runsAdapter, reportsAdapter, tenantsAdapter, etc.              │
└─────────────────────────────────────────────────────────────────┘
```

**Roles:** `ADMIN` | `TENANT_OWNER` | `STAFF` (platform) + tenant-scoped: `clinic_admin`, `receptionist`, `doctor`, `auditor`, `tenant_staff`
...
| Phase | Scope | Status |
|-------|-------|--------|
| **Phase 1** | Foundation (roles, entity types) | ✅ DONE |
| **Phase 2** | Sidebar restructure (Admin + Tenant) | ✅ DONE |
| **Phase 3** | Routing (all routes wired) | ✅ DONE |
| **Phase 4** | Modules (Admin dashboard, Tenant dashboard, Tenant details, Wizard, Staff, Agent, Agents, Support, Reports, Runs, Settings, Skills, Billing) | ✅ DONE |
| **Phase 5** | Guards & permissions | ✅ DONE |
| **Phase 6** | Data layer (seed, adapters) | ✅ DONE |
| **Phase 7A** | Toast, CSV export, Audit log, Date range, Command palette | ✅ DONE |
| **Phase 7B** | Global search, Scheduled reports, Calendar, 2FA, i18n/RTL | ✅ DONE |
| **Phase 7C** | Feature flags, Agent sandbox, Webhook log, PWA, A/B testing | ✅ DONE |
| **Phase 7 (7.1–7.8)** | Saved filters, Session mgmt, PII masking, GDPR export/delete, Maintenance mode, Health dashboard, Soft delete, Provider availability, Appointment reminders, PMS stubs, Multi-location, Custom prompts, Call replay, Intent analytics, Virtualized lists, Optimistic updates, Skeleton loading | ✅ DONE |
...
### Admin Portal
- **Overview:** KPIs, recent tenants, support snapshot, recent calls, system health
- **Tenants:** Directory, Add Tenant Wizard, Tenant detail (profile, settings, onboarding)
- **Agents:** List, assign to tenant, agent detail (skills, sync, runs)
- **Staff:** Cross-tenant staff table, add staff, CSV import
- **Support:** Unified inbox, filters, assign, reply
- **Calls:** Cross-tenant calls, call detail with transcript
- **Runs:** Agent runs list, run events debug view
- **Skills:** Coming Soon placeholder
- **Billing:** Cross-tenant plans, usage, revenue
- **Settings:** Admin users, integrations (Retell, webhooks), retention, scheduled reports
- **System:** Maintenance mode, health dashboard (Retell, DB, API, Stripe, Webhooks)

### Tenant Portal
- **Dashboard:** KPIs, agent status, recent calls, staff quick view, open tickets
- **Calls:** Call list, call detail, transcript, sentiment
- **Agent:** Status, skills, sync status
- **Staff:** Staff table, add staff, CSV import
- **Reports:** Outcome breakdown, performance metrics, sentiment, peak hours, outcomes over time, A/B comparison
- **Help Center:** Create ticket, my tickets, ticket chat thread
- **Bookings:** Bookings table, calendar view
- **Customers:** Customer list, customer detail
- **Alerts:** Alerts list
- **Billing:** Credits, usage
- **Settings:** Clinic profile, business hours, notifications, feature flags, PMS, appointment reminders

### Enhancements
- Toast notifications, CSV export, Audit log, Date range filters
- Command palette (⌘K), Global search
- Scheduled reports, Calendar view, Comparison views (tenants, time periods)
- Call analytics (sentiment, peak hours, outcomes over time)
- 2FA/MFA, i18n + RTL (en/ar)
- Feature flags, Agent sandbox, Webhook event log
- PWA / offline support, A/B testing for agents
- Saved filters, Session management, PII masking, GDPR export/delete
- Maintenance mode, Health dashboard
- Virtualized lists, Optimistic updates, Skeleton loading
...
| Path | Purpose |
|------|---------|
| `src/app/` | Routing, layout, guards, session, account modal |
| `src/modules/` | Feature modules: admin, dashboard, calls, agent, staff, bookings, customers, alerts, help, reports, billing, settings, tenant |
| `src/adapters/` | Data access layer (local + API); components never access seed directly |
| `src/shared/` | Shared UI, types, hooks |
| `src/components/` | Global: CommandPalette, MaintenanceBanner, OfflineBanner |
| `src/mock/` | seedData.ts (used only by adapters) |
| `src/mocks/` | initialData.ts (storage service) |

**Rule:** Component → Hook → Adapter → API/Seed. No direct seed access from components.
...
| [PHASE-7-MULTI-LOCATION] | Multi-location support | 7.5 ✅ DONE |
